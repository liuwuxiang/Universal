//顶部按钮事件

//排序按钮
$(document).ready(function(){
    $(".Sort").click(function(){
        if ($('#paixu').hasClass('grade-w-roll')) {
            $('#paixu').removeClass('grade-w-roll');
        } else {
            $('#paixu').addClass('grade-w-roll');
        }
    });
});

//健身活动按钮
$(document).ready(function(){
    $(".Brand").click(function(){
        if ($('#type_choose').hasClass('grade-w-roll')) {
            $('#type_choose').removeClass('grade-w-roll');
        } else {
            $('#type_choose').addClass('grade-w-roll');
        }
    });
});

//全城按钮
$(document).ready(function(){
    $(".Regional").click(function(){
        if ($('#quancheng').hasClass('grade-w-roll')) {
            $('#quancheng').removeClass('grade-w-roll');
        } else {
            $('#quancheng').addClass('grade-w-roll');
        }
    });
});


//点击某一项收缩其他两项
//全称按钮
$(document).ready(function(){
    $(".Regional").click(function(){
        if ($('#paixu').hasClass('grade-w-roll')) {
            $('#paixu').removeClass('grade-w-roll');
        };
    });
});
$(document).ready(function(){
    $(".Regional").click(function(){
        if ($('#type_choose').hasClass('grade-w-roll')) {
            $('#type_choose').removeClass('grade-w-roll');
        };
    });
});
$(document).ready(function(){
    $(".Brand").click(function(){
        if ($('#quancheng').hasClass('grade-w-roll')) {
            $('#quancheng').removeClass('grade-w-roll');
        };
    });
});
$(document).ready(function(){
    $(".Brand").click(function(){
        if ($('#paixu').hasClass('grade-w-roll')) {
            $('#paixu').removeClass('grade-w-roll');
        };
    });
});
$(document).ready(function(){
    $(".Sort").click(function(){
        if ($('#quancheng').hasClass('grade-w-roll')) {
            $('#quancheng').removeClass('grade-w-roll');
        };
    });
});
$(document).ready(function(){
    $(".Sort").click(function(){
        if ($('#type_choose').hasClass('grade-w-roll')) {
            $('#type_choose').removeClass('grade-w-roll');
        } ;

    });
});
//4567890



//排序选项点击事件
function Sorts(sbj){
    var arr = document.getElementById("paixu_ul").getElementsByTagName("li");
    for (var i = 0; i < arr.length; i++){
        var a = arr[i];
        a.style.color = "";
    };
    sbj.style.color = "#40E0D0";
    $('#paixu').removeClass('grade-w-roll');
    $(".Sort").text(sbj.innerText);
    
    setTimeout(function(){ 
    		getNearbyBusiness(-1,-2,sbj.id);
    }, 500);
}

//附近控件选项点击事件
function Nearbies(sbj){
    var arr = document.getElementById("quancheng_ul").getElementsByTagName("li");
    for (var i = 0; i < arr.length; i++){
        var a = arr[i];
        a.style.color = "";
    };
    sbj.style.color = "#40E0D0";
    $('#quancheng').removeClass('grade-w-roll');
    $(".Regional").text(sbj.innerText);
    
    setTimeout(function(){ 
    		getNearbyBusiness(-1,sbj.id,-1);
    }, 500);
}

//商家分类控件选项点击事件
function BusinessTypes(sbj,type_id){
    var arr = document.getElementById("type_choose_ul").getElementsByTagName("li");
    for (var i = 0; i < arr.length; i++){
        var a = arr[i];
        a.style.color = "";
    };
    sbj.style.color = "#40E0D0";
    $('#type_choose').removeClass('grade-w-roll');
    $(".Brand").text(sbj.innerText);
    
    setTimeout(function(){ 
    		getNearbyBusiness(type_id,-2,-1);
    }, 500);
}



























