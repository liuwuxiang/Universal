...
mui.plusReady(function() {
	mui.ready(function() {
		...
		var order_name = self.order_name;
		$('#type_order_refund').text(order_name);
		
		//需要退款的套餐类型
		$('#type_order_refund').text();
	...
	function wnkOrderRefund() {
		...
		if(refund_number <= 0){
			...
			toast(2, "打开loading");
			$.ajax({
				url: Main.url + "/app/v2.0.0/wnkOrderRefund", 
				type: "POST",
				dataType: 'json',
				data: {
...
function wnkOrderRefundBaseInformation() {
	toast(2, "打开loading");
	$.ajax({
		url: Main.url + "/app/v2.0.0/wnkOrderRefundBaseInformation",
		type: "POST",
		dataType: 'json',
		data: {
			"user_id": storage['user_id'],
	...
	function selectAllOrderRefundReasonRecord() {
		...
		$.ajax({
			url: Main.url + "/app/v2.0.0/selectAllOrderRefundReasonRecord",
			type: "POST",
			dataType: 'json',
			data: {
