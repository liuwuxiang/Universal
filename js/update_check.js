...
var newVer = '';
// 检测更新地址
var checkUrl    = Main.url + '/download/userCheck';
// 下载更新地址
var downloadUrl = Main.url + '/download/getUserUpdate';
// 下载apk地址
var downAPKloadUrl = Main.url + '/download?type=user';
...
function downWgt (){
    ...
    plus.downloader.createDownload( downloadUrl + "?ver=" + newVer, {filename:"_doc/update/"}, function(d,status){
        plus.nativeUI.closeWaiting();
        if ( status == 200 ) {
...
function downApk (){
    ...
    plus.downloader.createDownload( downAPKloadUrl, {filename:"_doc/update/"}, function(d,status){
        plus.nativeUI.closeWaiting();
        if ( status == 200 ) {
